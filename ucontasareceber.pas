...
uses uDataMobile, uRelatorioContasAReceber;
...
begin
   ...
   fRelatorioContasAReceber.qryRelatorio.Close;
   fRelatorioContasAReceber.qryRelatorio.SQL.Clear;
   fRelatorioContasAReceber.qryRelatorio.SQL.Add('select * from conta_a_receber');
   fRelatorioContasAReceber.qryRelatorio.SQL.Add('where data_vencimento >= :datainicio');
   fRelatorioContasAReceber.qryRelatorio.SQL.Add('and data_vencimento   <= :datafim');
   fRelatorioContasAReceber.qryRelatorio.ParamByName('datainicio').AsDate := DateTimePicker1.Date;
   fRelatorioContasAReceber.qryRelatorio.ParamByName('datafim').AsDate    := DateTimePicker2.Date;
   if cbStatus.ItemIndex = 1 then
   begin
      fRelatorioContasAReceber.qryRelatorio.SQL.Add('and status = :status ');
      fRelatorioContasAReceber.qryRelatorio.ParamByName('status').AsString := 'PAGO';
   end
   else
      if cbStatus.ItemIndex = 2 then
   begin
      fRelatorioContasAReceber.qryRelatorio.SQL.Add('and status = :status ');
      fRelatorioContasAReceber.qryRelatorio.ParamByName('status').AsString := 'PENDENTE';
   end;
   fRelatorioContasAReceber.qryRelatorio.Open;
